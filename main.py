...
from video_editing.dataset.dataset import VideoDataset
...
def load_video_frames(video_frames_path, sample_frames, channels=3, height=576, width=1024):
    frames = sorted(os.listdir(video_frames_path))
    selected_frames = frames[0:0+sample_frames]
    
    # Initialize a tensor to store the pixel values
    pixel_values = torch.empty((sample_frames, channels, height, width))
...
def main():
    ...
    if args.enable_xformers_memory_efficient_attention:
        if is_xformers_available():
            ...
            logger.info(f"xformer version: {xformers_version}")
    ...
    for name, para in unet.named_parameters():
        if ('temporal_transformer_block' in name) or ('position_net' in name) or ('transformer_block' in name):
    ...
    train_dataset = VideoDataset(width=args.width, height=args.height, sample_frames=args.num_frames)
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("SVD_Insertion", config=vars(args))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                # pixel values are raw images, shape: batch, num_frames, 3, 576, 1024
                pixel_values = batch["pixel_values"].to(weight_dtype).to(
                    accelerator.device, non_blocking=True
                )
                conditional_pixel_values = pixel_values[:, 0:1, :, :, :]

                # latents shape: batch, num_frames, 4, 72, 128
                latents = tensor_to_vae_latent(pixel_values, vae)
                
                gt_pixel_values = batch["gt_pixel_values"].to(weight_dtype).to(
                    accelerator.device, non_blocking=True
                )
                gt_latents = tensor_to_vae_latent(gt_pixel_values, vae)
                ...
                inp_noisy_latents = torch.cat(
                    [inp_noisy_latents, conditional_latents], dim=2)    # 在特征维度上cat，所有帧的特征都包含了第一帧的特征，应该是self-attention的时候有用？

                # check https://arxiv.org/abs/2206.00364(the EDM-framework) for more details.
                target = gt_latents
                
                added_cross_attention_args = {
                    "boxes": batch["bboxes"],
                    "boxes_masks": batch["bboxes_masks"],
                }
                ...
                model_pred = unet(
                    ...
                    added_cross_attention_args=added_cross_attention_args
                    ).sample
                ...
                #
                # loss += torch.mean(mask * denoised_target - target * mask)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if (
                        ...
                        # demo_bbox = torch.tensor(np.load('./demo.npy')[0:1, :])
                        ...
                        with torch.autocast(
                            ...
                            for val_img_idx in range(args.num_validation_images):
                                ...
                                video_frames = pipeline(
                                    load_image('./validation_data/demo.jpg').resize((args.width, args.height)),
                                    torch.tensor(np.load('./validation_data/demo.npy')[0:1, :]),
                                    load_video_frames('./validation_data/demo_video/', args.num_frames),
                                    height=args.height,
                                    width=args.width,
